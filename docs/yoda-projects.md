...
- It will keep the original data in a clean state available for every study that requires it. The different versions of this data (e.g., before and after audio is converted to a standard format) are kept and available.
...
When starting a new project, you will first set up a new datalad repository made to comply with YODA.
```bash
...
```
If datalad retrieves your files from the wrong target (e.g., online instead of from the local dataset), you can use the `datalad get --source NAME` to get from a particular sibling.
